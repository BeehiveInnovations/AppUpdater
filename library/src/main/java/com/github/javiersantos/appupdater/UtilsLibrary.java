...
class UtilsLibrary {
  ...
  static Boolean isUpdateAvailable(Update installedVersion, Update latestVersion) {
    ...
    if (latestVersion.getLatestVersion() == null || TextUtils
  ...
  static URL getUpdateURL(Context context, UpdateFrom updateFrom, GitHub gitHub) {
    ...
    switch (updateFrom) {
      default:
        res = String.format(Config.PLAY_STORE_URL, getAppPackageName(context),
          Locale.getDefault().getLanguage());
        break;
      case GITHUB:
        res = Config.GITHUB_URL + gitHub.getGitHubUser() + "/" + gitHub
          .getGitHubRepo() + "/releases";
        break;
      case AMAZON:
  ...
  private static String getRecentChanges(UpdateFrom updateFrom, Boolean isAvailable,
                                         String source) {
